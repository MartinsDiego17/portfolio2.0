const Card = ({ name, date, title, description, habilities, link }) => {
    ...
    return (
        <div className="w-[60%] max-sm:w-[90%] p-[2%] max-sm:p-[20px] transition duration-150 cardContainer mx-auto mb-[2%] max-sm:mb-[7%] pb-[4%] rounded-[15px]">
            ...
            <h2 className="mt-[2.5%] text-[#eee] max-sm:text-pretty">
                {title} - <a
            ...
            <ul className="flex mt-[2%] max-sm:mt-[4%] max-sm:block">
                {
                    habilities.map(hab => (
                        <li className="max-sm:w-fit max-sm:py-[5px] max-sm:px-[10px] text-[#ddd] text-[.8rem] mr-[2%] px-[2%] py-[1%] rounded-[5px] hab bg-[#121212]">#{hab}</li>
                    ))
                }
            </ul>
            <p className="mt-[2%] max-sm:mt-[4%] max-sm:text-pretty text-[#888] text-[.9rem]" >{description}</p>
        </div>
    )
}
